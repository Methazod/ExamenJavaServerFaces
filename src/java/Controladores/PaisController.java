...
public class PaisController implements Serializable {
    ...
    @EJB
    private Repositorio.PaisFacade ejbFacade;
    ...
    public static SelectItem[] getSelectedPaises(List<Pais> paises) {        
        SelectItem[] items = new SelectItem[paises.size()];
        int i = 0;        
        for (Pais pais : paises) {                        
            items[i++] = new SelectItem(pais, pais.getCodPais());
        }
        return items;
    }
    
    public SelectItem[] getPaises(){
        return getSelectedPaises(ejbFacade.findAll());
    }
}
